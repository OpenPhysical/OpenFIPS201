...
| VE05.16.01 | Discovery / pairing interactions as implemented | Draft | Pairing reference data is readable over contact after PIN and usable for VERIFY with P2=`98` only inside authenticated secure messaging on contactless. The pairing code is exactly 8 ASCII digits; a well-formed mismatch returns `6300`. |
...
| VE05.36C.01 | Interrupted GENERAL AUTHENTICATE chain rolls back | Draft | `OpenFIPS201GeneralAuthenticateSymmetricTest.unrelatedCommandAbortsIncompleteGeneralAuthenticateChain` proves that VERIFY executes after interruption, the abandoned fragment cannot resume, and a fresh GENERAL AUTHENTICATE succeeds. Attach the profile-matrix JUnit log to the frozen CAP evidence. |
